...
const PHRASES: Record<string, Record<string, string>> = {
  'en-es': {
    // Greetings
    'hello': 'hola', 'hi': 'hola', 'hey': 'oye', 'hi there': 'hola',
    'good morning': 'buenos días', 'good afternoon': 'buenas tardes',
    'good evening': 'buenas noches', 'good night': 'buenas noches',
    'how are you': 'cómo estás', 'how are you doing': 'cómo te va',
    "how's it going": 'qué tal', 'what\'s up': 'qué pasa',

    // Farewells
    'goodbye': 'adiós', 'bye': 'adiós', 'bye bye': 'adiós',
    'see you': 'nos vemos', 'see you later': 'hasta luego',
    'see you soon': 'hasta pronto', 'take care': 'cuídate',

    // Courtesy
    'thank you': 'gracias', 'thanks': 'gracias', 'thank you very much': 'muchas gracias',
    'thanks a lot': 'muchas gracias', 'please': 'por favor',
    'you\'re welcome': 'de nada', 'no problem': 'no hay problema',
    'sorry': 'lo siento', 'excuse me': 'disculpe', 'pardon': 'perdón',

    // Basic responses
    'yes': 'sí', 'no': 'no', 'okay': 'está bien', 'ok': 'está bien',
    'of course': 'por supuesto', 'sure': 'claro', 'maybe': 'quizás',
    'i understand': 'entiendo', 'i don\'t understand': 'no entiendo',
    'can you repeat': 'puede repetir', 'can you repeat that': 'puede repetir eso',
    'i don\'t know': 'no sé', 'i think so': 'creo que sí',

    // Introductions
    'nice to meet you': 'mucho gusto', 'pleased to meet you': 'encantado',
    'my name is': 'me llamo', 'what is your name': 'cómo te llamas',
    'what\'s your name': 'cómo te llamas', 'where are you from': 'de dónde eres',
    'i am from': 'soy de', 'i\'m from': 'soy de',

    // Questions
    'what': 'qué', 'where': 'dónde', 'when': 'cuándo', 'why': 'por qué',
    'how': 'cómo', 'who': 'quién', 'which': 'cuál',
    'how much': 'cuánto', 'how much is it': 'cuánto cuesta',
    'how many': 'cuántos', 'what time is it': 'qué hora es',
    'where is': 'dónde está', 'what is this': 'qué es esto',
    'do you speak english': 'hablas inglés',

    // Needs & Wants
    'i want': 'quiero', 'i need': 'necesito', 'i would like': 'me gustaría',
    'i need help': 'necesito ayuda', 'help me': 'ayúdame', 'help': 'ayuda',
    'can i have': 'puedo tener', 'can you help me': 'puedes ayudarme',

    // Common words
    'water': 'agua', 'food': 'comida', 'bathroom': 'baño',
    'hotel': 'hotel', 'restaurant': 'restaurante', 'hospital': 'hospital',
    'police': 'policía', 'doctor': 'médico', 'money': 'dinero',
    'the bill please': 'la cuenta por favor', 'menu': 'menú',
    ...
    'i love you': 'te quiero', 'i like it': 'me gusta', 'i don\'t like it': 'no me gusta',
    'good': 'bueno', 'bad': 'malo', 'very good': 'muy bueno', 'great': 'genial',
...
async function translateMyMemory(text: string, from: string, to: string): Promise<string | null> {
  try {
    ...
    if (data.responseStatus === 200 && data.responseData?.translatedText) {
      ...
      if (translated.toUpperCase().includes('INVALID') || translated.toUpperCase().includes('LIMIT')) return null
      return translated
    }
    return null
  } catch { return null }
...
async function translateLibre(text: string, from: string, to: string): Promise<string | null> {
  const instances = ['https://translate.fedilab.app', 'https://translate.adminforge.de']
