...
async function sendAlert(health: HealthResponse, isRecovery: boolean = false) {
  ...
  } else if (ALERT_WEBHOOK_URL?.includes('slack') || ALERT_WEBHOOK_URL?.includes('hooks.slack')) {
...
export async function GET(request: Request) {
  ...
  if (process.env.CRON_SECRET && authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    // Allow access without secret for testing, but log it
    console.log('Cron request without valid secret')
  }
