...
async function checkService(
  ...
  try {
    ...
    return [
      name,
      { status: "error", latency: Date.now() - start, error: err.message },
    ];
  }
}
...
export async function GET() {
  ...
  try {
    ...
    const checks = await Promise.all([
      ...
      checkService("lingva", async () => {
        const res = await fetch("https://lingva.ml/api/v1/en/es/hello", {
          headers: { "User-Agent": "Voxxo/14.1 (Health Check)" },
          signal: AbortSignal.timeout(5000),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
    ...
    const translationOk = translationApis.some(
      (api) => services[api]?.status === "ok",
    );
